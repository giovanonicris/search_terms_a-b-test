...
main_csv_path = os.path.join(output_dir, 'enterprise_risk_sampleset_b.csv')

# encode-decode search terms
read_file = pd.read_csv('enterprise_risk_list_b.csv', encoding='utf-8')
...
# re-decode terms
joined_df = pd.merge(alerts, read_file, on='SEARCH_TERMS', how='left')
joined_df['ENCODED_TERMS'] = joined_df['SEARCH_TERMS'].apply(lambda x: int.from_bytes(x.encode('utf-8'), byteorder='little') if pd.notnull(x) else None)
